...
export default function SeatingBaggagePage() {
    ...
    const handleSeatSelection = (flightIndex, travellerIndex, seatName) => {
        // Update the selected seat for the specific traveller on the specific flight
        // [[A3, B2], [C1, D4]]
    ...
    function SeatSelectionMenu({ flight }) {
        ...
        const travellerLabel = (i) => {
            const t = travellers[i];
            return (t && t.firstName) ? `${t.firstName} ${t.lastName}` : `Traveller ${i + 1}`;
        };

        const optionsForTraveller = (travellerIndex) => {
            return availableSeats.filter(seat =>
                seat.name === selectedForFlight[travellerIndex] ||
                !selectedForFlight.includes(seat.name)
            );
        };
        ...
        return (
            <div className="seat-selection-menu">
                <h5>Select Seats</h5>
                {Array.from({ length: tripData.travellerCount }).map((_, travellerIndex) => (
                    <div key={travellerIndex} className="seat-options" style={{ marginBottom: '8px' }}>
                        <p>{travellerLabel(travellerIndex)}</p>
                        <select
                            value={selectedForFlight[travellerIndex] || ''}
                            onChange={(e) => handleSeatSelection(flightIndex, travellerIndex, e.target.value)}
                        >
                            <option value="">-- Select a seat --</option>
                            {optionsForTraveller(travellerIndex).map((seat) => (
                ...
                <p>Selected Seats: {selectedForFlight.filter(Boolean).join(', ') || 'None'}</p>
            </div>
        );
    }
